...
class CurrencyConverter:
    ...
    def set_exchange_rate(self, from_currency: str, to_currency: str, rate: float):
        ...
        if rate <= 0:
            raise CurrencyError(
                f"Exchange rate must be positive: {rate}", exchange_rate=rate
            )
    ...
    def convert(
        self, amount: Union[Decimal, float, int], from_currency: str, to_currency: str
    ) -> Decimal:
        ...
        if amount < 0:
            raise ValidationError(
                f"Amount cannot be negative: {amount}", field="amount", value=amount
            )
    ...
    def convert_to_sek(
        self, amount: Union[Decimal, float, int], currency: str
    ) -> Decimal:
...
def convert_currency(
    amount: Union[Decimal, float, int], from_currency: str, to_currency: str
) -> Decimal:
    """Convert amount using the global converter."""
    return _currency_converter.convert(amount, from_currency, to_currency)


def convert_to_sek(amount: Union[Decimal, float, int], currency: str) -> Decimal:
