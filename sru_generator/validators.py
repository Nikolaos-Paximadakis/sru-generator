...
from .currency import is_supported_currency, CurrencyAmount
...
class StringValidator(Validator):
    ...
    def __init__(self, field_name: str, max_length: Optional[int] = None, 
                 min_length: Optional[int] = None, pattern: Optional[str] = None,
                 required: bool = True):
    ...
    def _validate_value(self, value: Any, context: Dict[str, Any]) -> str:
        if not isinstance(value, str):
            raise ValidationError(
                f"Field '{self.field_name}' must be a string, got {type(value).__name__}",
                field=self.field_name,
                value=value
            )
...
class TradeDataValidator:
    ...
    def validate_trade_data(self, trade_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
